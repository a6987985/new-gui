...
from PyQt5.QtWidgets import QCheckBox, QHBoxLayout, QLabel, QPushButton, QSizePolicy, QVBoxLayout, QWidget

from new_gui.presentation.assets.icon_factory import build_panel_stack_icon, build_side_panel_icon
from new_gui.presentation.styles.top_button_styles import build_neutral_top_button_style
...
DEP_GRAPH_SWITCH_WIDTH = 130
DEP_GRAPH_SWITCH_SPACING = ROW1_BUTTON_SPACING


class DependencyGraphToggleSwitch(QCheckBox):
    """Custom ON/OFF switch styled like a native rounded slider."""
    ...
    def hitButton(self, pos):
        """Treat the full slider rectangle as the active click target."""
    ...
    def paintEvent(self, event):
        ...
        track_color = QColor("#2f7adf" if enabled else "#5a5f66")
        border_color = QColor("#2c6ec7" if enabled else "#5a5f66")
        knob_x = rect.right() - 22 if enabled else rect.left() + 2
        knob_rect = rect.adjusted(knob_x - rect.left(), 2, -(rect.width() - (knob_x - rect.left()) - 20), -2)

        painter.setPen(QPen(border_color, 1))
        painter.setBrush(QBrush(track_color))
        painter.drawRoundedRect(rect, 12, 12)

        painter.setPen(QPen(QColor("#cfd3d7"), 1))
        painter.setBrush(QBrush(QColor("#f3f4f6")))
        painter.drawEllipse(knob_rect)


def _build_dependency_graph_switch_control(window) -> QWidget:
    """Build the text+switch control used to toggle Dependency Graph mode."""
    container = QWidget()
    ...
    label = QLabel("Dependency Graph", container)
    label.setStyleSheet("color: #51697f; font-size: 12px; font-weight: 600;")
    layout.addWidget(label)

    toggle = DependencyGraphToggleSwitch(container)
    toggle.setChecked(False)
    toggle.toggled.connect(lambda checked: window.show_dependency_graph() if checked else window.show_main_view_tab())
    layout.addWidget(toggle)

    window.dependency_graph_toggle = toggle
    window._dependency_graph_toggle_label = label
    window._dependency_graph_toggle_container = container
    return container
...
def _get_row2_leading_offset() -> int:
    ...
    if TOP_LEFT_TOGGLE_BUTTON_COUNT <= 0:
        return 0
    return (
        DEP_GRAPH_SWITCH_WIDTH
        + DEP_GRAPH_SWITCH_SPACING
...
def _build_top_button_row_widget(
    ...
    if row_role == "row1":
        row_layout.addWidget(_build_dependency_graph_switch_control(window))
