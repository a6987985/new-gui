...
from new_gui.shared.config.settings import FADE_IN_DURATION_MS, WINDOW_HEIGHT, WINDOW_WIDTH
from new_gui.presentation.views.builders import top_panel_builder
from new_gui.presentation.presenters import theme_controller


def should_enable_window_fade_animation(system_name: str = None) -> bool:
    """Return whether the top-level window should use opacity fade-in."""
    effective_system = (system_name or platform.system() or "").strip()
    return effective_system.lower() != "linux"
...
def init_window(window) -> None:
    ...
    if should_enable_window_fade_animation():
