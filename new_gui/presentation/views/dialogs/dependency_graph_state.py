...
class DependencyGraphStateMixin:
    ...
    def _apply_initial_view_state(self):
        ...
        pending_viewport_state = getattr(self, "_pending_viewport_state", None)
        pending_selected_target = getattr(self, "_pending_selected_target", None)
        self._pending_selected_target = None
        if pending_viewport_state:
            self._pending_viewport_state = None
            self._restore_viewport_state(pending_viewport_state)
            restored_node = self._resolve_graph_node(pending_selected_target)
            if restored_node and restored_node in self.node_rects:
                self.select_node(restored_node)
            return
