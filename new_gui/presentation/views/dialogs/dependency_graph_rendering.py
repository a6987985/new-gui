...
from new_gui.presentation.assets import icon_factory
...
class DependencyGraphRenderingMixin:
    ...
    def _is_adjacent_level_edge(self, source_node, target_node):
        """Return whether the edge connects two immediately adjacent levels."""
        source_level = self.node_levels.get(source_node)
        target_level = self.node_levels.get(target_node)
        if source_level is None or target_level is None:
            return False
        return abs(int(source_level) - int(target_level)) == 1
    ...
    def _show_selected_node_edges(self, node_name):
        ...
        for edge_item in self.edge_items:
            ...
            target = edge_item.data(2)
            if (
                self._edge_touches_node(edge_item, node_name)
                and self._is_adjacent_level_edge(source, target)
            ):
    ...
    def _collect_canonical_trace(self, node_name, direction):
        """Collect trace nodes from the canonical dependency trace payload."""
        highlighted_nodes = {node_name}
        highlighted_nodes.update(self._get_trace_targets(node_name, direction))
        highlighted_edges = set()
        for source, target in self.graph_data.get("edges", []):
            if source in highlighted_nodes and target in highlighted_nodes:
    ...
    def _trace_dependencies(self, node, direction):
        ...
        nodes_to_highlight, highlighted_edges = self._collect_canonical_trace(node, direction)
        related_node_set = set(nodes_to_highlight)
        related_node_set.discard(node)
