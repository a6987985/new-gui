...
class HeaderFilterLineEdit(QLineEdit):
    ...
    def setup_search_options_ui(self) -> None:
        ...
        self.setTextMargins(10, 0, right_padding, 0)
...
class FilterHeaderView(QHeaderView):
    ...
    def _update_filter_geometry(self):
        ...
        if parent_widget is self.viewport():
            top_left = content_rect.topLeft() + QPoint(2, 2)
        else:
            try:
    ...
    def _ensure_filter_edit(self):
        ...
        parent_widget = self.viewport() or self
    ...
    def paintSection(self, painter: QPainter, rect: QRect, logical_index: int):
        ...
        elided_text = font_metrics.elidedText(
            header_text,
            Qt.ElideRight,
            max(0, content_rect.width() - 8),
        )
        left_margin, right_margin = self.get_section_text_margins()
        text_rect = content_rect.adjusted(left_margin, 0, -right_margin, 0)
        ...
        if text_rect.width() < font_metrics.horizontalAdvance(header_text):
