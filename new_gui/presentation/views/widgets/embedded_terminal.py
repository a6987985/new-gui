...
from new_gui.infrastructure.repositories.flow_background import (
    choose_terminal_foreground,
    normalize_background_color,
)
from new_gui.infrastructure.repositories import terminal_embed_backend
from new_gui.presentation.styles.output_panel_styles import build_embedded_terminal_style


TERMINAL_FRAME_BORDER = 1
TERMINAL_BOTTOM_SAFE_PADDING = 6
...
class EmbeddedTerminalWidget(QWidget):
    ...
    def _build_xterm_arguments(self):
        ...
        _, _, child_width, child_height = self._embedded_child_geometry()
    ...
    def _embedded_child_geometry(self) -> tuple:
        """Return the embedded child window geometry inside the host frame."""
        inset = TERMINAL_FRAME_BORDER
        bottom_padding = TERMINAL_BOTTOM_SAFE_PADDING
        child_width = max(1, self._terminal_host.width() - (inset * 2))
        child_height = max(
            1,
            self._terminal_host.height() - (inset * 2) - bottom_padding,
        )
        return inset, inset, child_width, child_height
    ...
    def _sync_embedded_window_geometry(self) -> None:
        ...
        x_pos, y_pos, child_width, child_height = self._embedded_child_geometry()
