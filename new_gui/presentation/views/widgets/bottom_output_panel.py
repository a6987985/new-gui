...
from new_gui.presentation.assets.icon_factory import (
    build_terminal_content_fill_icon,
    build_terminal_follow_run_icon,
)
...
class BottomOutputPanel(QWidget):
    ...
    terminal_content_fill_changed = pyqtSignal(bool)
    ...
    def __init__(self, parent=None):
        ...
        self._terminal_content_fill_button = QToolButton(self._terminal_controls)
        self._terminal_content_fill_button.setObjectName("terminalContentFillButton")
        self._terminal_content_fill_button.setCheckable(True)
        self._terminal_content_fill_button.setAutoRaise(False)
        self._terminal_content_fill_button.setFixedSize(30, 30)
        self._terminal_content_fill_button.setIcon(build_terminal_content_fill_icon())
        self._terminal_content_fill_button.setIconSize(QSize(16, 16))
        self._terminal_content_fill_button.toggled.connect(
            self._on_terminal_content_fill_toggled
        )
        terminal_controls_layout.addWidget(self._terminal_content_fill_button)
    ...
    def is_terminal_content_filled(self) -> bool:
        """Return whether the terminal fills the content area."""
        return self._terminal_content_fill_button.isChecked()

    def set_terminal_content_filled(self, enabled: bool) -> None:
        """Update the terminal content-fill toggle state without re-emitting."""
        was_blocked = self._terminal_content_fill_button.blockSignals(True)
        self._terminal_content_fill_button.setChecked(bool(enabled))
        self._terminal_content_fill_button.blockSignals(was_blocked)
        self._refresh_terminal_content_fill_tooltip()
    ...
    def _on_terminal_content_fill_toggled(self, enabled: bool) -> None:
    ...
    def _refresh_terminal_content_fill_tooltip(self) -> None:
        """Update the tooltip for the terminal content-fill toggle."""
        if self._terminal_content_fill_button.isChecked():
            self._terminal_content_fill_button.setToolTip(
                "Restore terminal to the bottom panel"
            )
        else:
            self._terminal_content_fill_button.setToolTip(
                "Fill the content area with the terminal"
            )
