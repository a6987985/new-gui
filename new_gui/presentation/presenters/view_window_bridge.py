...
from new_gui.presentation.presenters import output_controller, runtime_controller
...
class ViewWindowBridge:
    ...
    def is_all_status_view(self, value):
        if value:
            view_mode_state.set_tree_mode_all_status(self._window)
        else:
            view_mode_state.set_tree_mode_main(self._window)
    ...
    def filter_tree(self, text, search_options=None):
        self._window.filter_tree(text, search_options=search_options)

    def filter_tree_by_targets(self, targets):
        self._window.filter_tree_by_targets(targets)

    def apply_status_filter(self, status, update_tab=True):
        self._window._apply_status_filter(status, update_tab=update_tab)
    ...
    def set_search_mode(self, active):
        if active:
            search_text = self.header_filter_text()
            view_mode_state.set_tree_mode_search(
                self._window,
                search_text,
                self.header_filter_options(),
            )
        else:
            view_mode_state.clear_search_state(self._window)
    ...
    def set_cached_target_data(self, run_name, targets_by_level, collapsible_groups):
        if hasattr(self._window, "_cache_manager"):
            self._window._cache_manager.set_targets_cache(run_name, targets_by_level, collapsible_groups)
            self._window.cached_targets_by_level = self._window._state.run_cache.targets_by_level
            self._window._cached_targets_run = self._window._state.run_cache.cached_targets_run
            self._window.cached_collapsible_target_groups = self._window._state.run_cache.collapsible_target_groups
            self._window._cached_collapsible_target_groups_run = self._window._state.run_cache.cached_collapsible_groups_run
            return
    ...
    def activate_selected_run_view(self, current_run, invalidate_snapshot=True, presentation_snapshot=None):
        self._window._activate_selected_run_view(
            current_run,
    ...
    def has_dependency_watcher(self):
        return hasattr(self._window, "dependency_watcher")

    def setup_dependency_watcher(self):
        self._window.setup_dependency_watcher()
    ...
    def mark_dependency_graph_dirty(self):
        if hasattr(self._window, "_mark_dependency_graph_dirty"):
            self._window._mark_dependency_graph_dirty()

    def clear_cached_target_data(self):
        if hasattr(self._window, "_cache_manager"):
            self._window._cache_manager.clear_targets_cache()
            self._window.cached_targets_by_level = self._window._state.run_cache.targets_by_level
            self._window._cached_targets_run = self._window._state.run_cache.cached_targets_run
            self._window.cached_collapsible_target_groups = self._window._state.run_cache.collapsible_target_groups
            self._window._cached_collapsible_target_groups_run = self._window._state.run_cache.cached_collapsible_groups_run
            return
        self._window.cached_targets_by_level = {}
        self._window._cached_targets_run = ""
        self._window.cached_collapsible_target_groups = {}
        self._window._cached_collapsible_target_groups_run = ""

    def capture_main_view_snapshot(self):
        self._window._capture_main_view_snapshot()

    def append_search_rows(self, grouped_rows):
        self._window._append_search_rows(grouped_rows)

    def show_tree_and_tab_bar(self):
        self._window.tree.show()
        self._window.tab_bar.show()
    ...
    def consume_pending_dependency_refresh(self):
        pending = bool(getattr(self._window, "_pending_dependency_refresh", False))
        self._window._pending_dependency_refresh = False
        return pending
