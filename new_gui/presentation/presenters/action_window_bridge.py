...
class ActionWindowBridge:
    ...
    def request_action_refresh(self, search_context: dict, command: str = "") -> None:
        """Queue one async action refresh request onto the GUI thread."""
        if hasattr(self._window, "request_action_refresh"):
            self._window.request_action_refresh(search_context, command=command)

    def is_graph_mode_active(self) -> bool:
        """Return whether the dependency-graph content mode is active."""
        return view_mode_state.get_content_mode(self._window) == "graph"

    def trace_graph_dependencies(self, direction: str) -> bool:
        """Trigger graph trace actions in graph mode when the panel is available."""
        panel = getattr(self._window, "_dependency_graph_panel", None)
        if panel is None:
            return False
