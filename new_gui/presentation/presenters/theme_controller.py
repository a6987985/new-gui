...
def apply_theme(window, theme_name, announce: bool = True) -> None:
    ...
    if hasattr(window, "left_sidebar") and hasattr(window.left_sidebar, "refresh_background_style"):
        window.left_sidebar.refresh_background_style()
    ...
    agent_panel = getattr(window, "_agent_panel", None)
    if agent_panel is not None and hasattr(agent_panel, "apply_theme"):
        try:
