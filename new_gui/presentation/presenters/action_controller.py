...
from new_gui.application.use_cases import action_flow
...
from new_gui.model.services import tree_editing
from new_gui.model.services import view_modes
from new_gui.application.use_cases import tune_workflow
...
from new_gui.presentation.presenters.action_window_bridge import ActionWindowBridge
from new_gui.presentation.views.dialogs.tune_dialogs import CopyTuneSelectDialog, SelectTuneDialog
from new_gui.presentation.views.widgets.cell_option_popup import CellOptionPopup
...
def refresh_after_action(window, search_context) -> None:
    ...
    ui = _bridge(window)
    runtime_controller.start_action_refresh_window(window, source="action")
...
def start(window, action) -> None:
    ...
    elif action_request.get("detached", False):
        result = action_flow.execute_shell_command_detached(
            action_request["argv"],
            action_request["cwd"],
        )
        if result.get("error") is not None:
            log_action_result(window, action_request["command"], result, include_returncode=True)
        else:
            ui.append_ui_log(
                "INFO",
                "action",
                "Command started in detached mode.",
                command=action_request["command"],
                details=f"PID: {result.get('pid', 'N/A')}",
            )
        logger.info(
            f"Detached action dispatched, requesting UI refresh: {action_request['command']}",
            extra={"ui_source": "action"},
        )
        ui.request_action_refresh(search_context, command=action_request["command"])
    else:
        def run_command():
            try:
                result = action_flow.execute_shell_command(
                ...
                logger.info(
                    f"Async action finished, requesting UI refresh: {action_request['command']}",
                    extra={"ui_source": "action"},
                )
                ui.request_action_refresh(search_context, command=action_request["command"])
...
def retrace_tab(window, inout) -> None:
    ...
    ui = _bridge(window)
    if ui.is_graph_mode_active():
        if not ui.trace_graph_dependencies(inout):
            logger.warning("Graph trace requested without an active graph panel", extra={"ui_source": "action"})
        return
    ...
    if view_modes.get_active_view_mode(window) == "main":
        window._trace_return_scroll_value = window.tree.verticalScrollBar().value()
    ...
    view_mode_state.set_tree_mode_trace(window, selected_target, inout)
