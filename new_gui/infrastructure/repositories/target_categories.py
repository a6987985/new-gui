...
def ensure_shared_target_stage_file(
    ...
    file_name: str = DEFAULT_TARGET_STAGE_FILE_NAME,
    create_gui_dir: bool = False,
    copy_target_file: bool = False,
    ...
    if not gui_dir_existed:
        if not create_gui_dir:
            return target_file, created_gui_dir, copied_target_file, error_message
        try:
            os.makedirs(gui_dir, exist_ok=True)
            created_gui_dir = os.path.isdir(gui_dir)
        except OSError as exc:
            return target_file, created_gui_dir, copied_target_file, str(exc)

    if copy_target_file and not os.path.exists(target_file) and os.path.isfile(source_file):
        try:
