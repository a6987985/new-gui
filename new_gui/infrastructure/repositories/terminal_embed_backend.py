...
def estimate_terminal_cell_size(point_size: int = 11) -> Tuple[int, int]:
    """Estimate monospace terminal cell size for the embedded xterm font."""
...
def estimate_terminal_geometry(host_width: int, host_height: int, point_size: int = 11) -> Tuple[int, int]:
    ...
    cell_width, cell_height = estimate_terminal_cell_size(point_size)
    usable_width = max(
        MIN_TERMINAL_COLUMNS * cell_width,
        host_width - TERMINAL_PIXEL_MARGIN,
    )
    usable_height = max(
        MIN_TERMINAL_ROWS * cell_height,
        host_height - TERMINAL_PIXEL_MARGIN,
    )
    columns = max(MIN_TERMINAL_COLUMNS, usable_width // cell_width)
    rows = max(MIN_TERMINAL_ROWS, usable_height // cell_height)
    return columns, rows
