...
from new_gui.presentation.presenters import action_controller
from new_gui.presentation.views.dialogs.dependency_graph import DependencyGraphDialog
from new_gui.presentation.views.widgets.bottom_output_panel import BottomOutputPanel, GuiLogEntry
...
_LEGACY_IMPORT_PATTERNS = (
    re.compile(r"^\s*from\s+new_gui\.services(?:\.|\s+import\s+)"),
    re.compile(r"^\s*import\s+new_gui\.services(?:\.|$)"),
    re.compile(r"^\s*from\s+new_gui\.ui(?:\.|\s+import\s+)"),
    re.compile(r"^\s*import\s+new_gui\.ui(?:\.|$)"),
    re.compile(r"^\s*from\s+new_gui\.config\.settings\s+import\s+"),
    re.compile(r"^\s*import\s+new_gui\.config\.settings(?:\.|$)"),
    re.compile(r"^\s*from\s+new_gui\.tools\.governance_smoke\s+import\s+"),
    re.compile(r"^\s*import\s+new_gui\.tools\.governance_smoke(?:\.|$)"),
)


def run_legacy_import_guard() -> None:
    """Fail when any source file still imports deprecated pre-MVP paths."""
    offenders: list[str] = []
    for source_path in _python_sources():
        with source_path.open("r", encoding="utf-8", errors="ignore") as handle:
            for line_number, line_text in enumerate(handle, start=1):
                if any(pattern.search(line_text) for pattern in _LEGACY_IMPORT_PATTERNS):
                    offenders.append(f"{source_path}:{line_number}: {line_text.strip()}")

    if offenders:
        preview = "\n".join(offenders[:20])
        more = "" if len(offenders) <= 20 else f"\n... and {len(offenders) - 20} more"
        raise SmokeFailure(
            "Legacy import path detected. Use MVP paths instead.\n"
            f"{preview}{more}"
        )
...
def smoke_main_content_graph_tabs(window: MainWindow, app: QApplication) -> None:
    """Verify the embedded graph-tab flow lazy-loads and restores sidebar visibility."""
    ...
    _require(hasattr(window, "dependency_graph_toggle"), "Dependency graph switch is missing.")
    ...
    _require(window.left_sidebar.isVisible(), "Sidebar should stay visible while graph mode is active.")
    _require(window.dependency_graph_toggle.isChecked(), "Dependency graph switch did not reflect graph mode.")
