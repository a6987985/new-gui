...
CrossRunPresentationSnapshot = Dict[str, object]
...
def _build_expandable_row_identity(row_items) -> Tuple[str, str, str, Tuple[str, ...]]:
    """Build a stable row identity for cross-run expansion-state restore."""
    level_item = row_items[0] if row_items else None
    target_item = row_items[1] if len(row_items) > 1 else None
    level_text = (level_item.text() if level_item is not None else "") or ""
    row_kind = tree_rows.get_row_kind(target_item) or ""
    row_label = (target_item.text() if target_item is not None else "") or ""
    represented_targets = tuple(tree_rows.get_row_targets(target_item))
    return row_kind, level_text, row_label, represented_targets


def capture_cross_run_presentation_snapshot(model, tree) -> CrossRunPresentationSnapshot:
    """Capture expansion and scroll state that can be replayed on another run tree."""
    expansion_entries: List[Dict[str, object]] = []

    def walk_rows(parent_item=None) -> None:
        row_count = parent_item.rowCount() if parent_item is not None else model.rowCount()
        for row in range(row_count):
            row_items = tree_rows.get_row_items(model, row, parent_item)
            level_item = row_items[0] if row_items else None
            if level_item is None or not level_item.hasChildren():
                continue

            level_index = level_item.index()
            expansion_entries.append(
                {
                    "identity": _build_expandable_row_identity(row_items),
                    "expanded": bool(tree.isExpanded(level_index)),
                }
            )
...
def restore_cross_run_presentation_snapshot(
    model,
    tree,
    snapshot: CrossRunPresentationSnapshot,
) -> bool:
    """Best-effort restore of expansion and scroll state across different run trees."""
    if not snapshot:
        return False

    state_by_identity = {
        tuple(entry.get("identity", ())): bool(entry.get("expanded", False))
        for entry in snapshot.get("expansion_entries", [])
    }
        ...
        def walk_rows(parent_item=None) -> None:
            ...
            for row in range(row_count):
                ...
                if identity in state_by_identity:
                    tree.setExpanded(level_item.index(), state_by_identity[identity])
                    restored_any = True
                walk_rows(level_item)
    ...
    tree.verticalScrollBar().setValue(int(snapshot.get("scroll", 0)))
    return restored_any
